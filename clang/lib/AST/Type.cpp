...
DependentTemplateSpecializationType::DependentTemplateSpecializationType(
    ...
    : TypeWithKeyword(
          Keyword, DependentTemplateSpecialization, Canon,
          toTypeDependence(Name.getDependence()),
          // todo [merge:yukino:maybe-revert]
          /*ConstevalOnly=*/!Canon.isNull() ? Canon->isConstevalOnly() : false),
      Name(Name) {
          ...
          (Tag->isDependentType() ? TypeDependence::DependentInstantiation
              ...
              (Qualifier
                   ? toTypeDependence(Qualifier.getDependence() &
                                      ~NestedNameSpecifierDependence::Dependent)
                   : TypeDependence{}),
          isa<RecordDecl>(Tag) && cast<RecordDecl>(Tag)->isConstevalOnly()),
...
TemplateSpecializationType::TemplateSpecializationType(
    ...
    : TypeWithKeyword(Keyword, TemplateSpecialization,
                      ...
                      !Underlying.isNull() ? Underlying->isConstevalOnly()
                                           : false),
      Template(T) {
...
bool MemberPointerType::isSugared() const {
  ...
  if (getQualifier().getKind() == NestedNameSpecifier::Kind::Splice ||
      getQualifier().getKind() == NestedNameSpecifier::Kind::SpliceWithTemplate)
    return true;
  assert(!D1 == !D2);
