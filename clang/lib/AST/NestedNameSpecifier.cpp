...
bool NestedNameSpecifier::isFullyQualified() const {
  ...
  case NestedNameSpecifier::Kind::SpliceWithTemplate:
    // A Splice NNS is a root component (it has no prefix stored).
    // Roots are only fully qualified if they are Kind::Global.
    return false;
  }
...
NestedNameSpecifierDependence NestedNameSpecifier::getDependence() const {
  ...
  case Kind::Namespace: {
    ...
    if (const NamespaceAliasDecl *Alias = dyn_cast<NamespaceAliasDecl>(NS)) {
...
void NestedNameSpecifierLocBuilder::MakeSpliceScopeSpecifier(
  ...
  // todo [merge:yukino:maybe-revert]
  Representation = NestedNameSpecifier(/*Context, */ Template, Splice);
