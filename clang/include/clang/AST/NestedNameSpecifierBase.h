...
class NestedNameSpecifier {
  ...
  enum class StoredKind {
    ...
    SpliceSpecifier,
    SpliceSpecifierWithTemplate,
    NumKinds,
  };
  // Extend `FlagBits` to 3 to accomodate all `StoredKind`s
  // todo [merge:yukino:todo,"we need 4 bits for flags but NestedNameSpecifier
  //   are all 8-byte aligned...So we can only make everything 16-byte aligned,
  //   from Decl, Type (already 16-aligned), Namespace, to Splice."]
  static constexpr uintptr_t FlagBits = 3, FlagMask = (1u << FlagBits) - 1u,
  // One extra bit for potential `UnderlyingTemplate`
                             FlagOffset = 1, PtrOffset = FlagBits + FlagOffset,
                             PtrMask = (1u << PtrOffset) - 1u;

  static_assert((1u << FlagBits) >= (unsigned)StoredKind::NumKinds,
                "NestedNameSpecifier::FlagBits is too small to store all "
                "StoredKind values! "
                "Increase FlagBits.");
...
public:
  ...
  /// Builds a nested name specifier that names a splice.
  explicit inline NestedNameSpecifier(
  ...
  // todo [merge:yukino:maybe-revert,impl]
  /// For a nested-name-specifier that refers to a splice specifier, retrieve
  /// the splice specifier.
  inline const SpliceSpecifier *getAsSplice() const;

  // todo [merge:yukino:maybe-revert,impl]
  inline const SpliceSpecifier *getSplice() const;
